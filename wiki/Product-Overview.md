# Product Overview Documentation

## Product Summary
Bloom is a private, supportive period-tracking companion that helps users log cycles and symptoms, receive personalized insights, and manage period-related health with confidence. The app stores all data locally on the device to protect sensitive health information.

## Goals
- Enable accurate period and symptom tracking with minimal friction.
- Provide personalized cycle predictions based on user history, not generic averages.
- Offer timely guidance (e.g., pain management advice and anomaly alerts).
- Preserve user privacy by keeping data local with optional export/import.
- Deliver a reliable, offline-capable mobile experience via PWA.

## Target Users
- People who menstruate and want a privacy-first tracker.
- Users seeking symptom correlation and cycle awareness.
- Users who prefer an installable, offline-capable mobile web app.

## Key Value Propositions
- Privacy-first data storage (local only by default).
- Personalized predictions driven by real user history.
- Practical health guidance when symptoms are logged.
- Visual, easy-to-read calendar and status insights.
- PWA installability with offline support.

## Core Features (MVP)
- Period logging (start/end, flow intensity).
- Cycle calendar with highlights and predictions.
- Personalized cycle predictions based on history.
- Symptom tracking (cramps, headaches, mood, bloating, fatigue, breast tenderness).
- Pain management advice contextual to symptoms.
- Anomaly alerts for irregular or concerning cycles.
- Reminders/alarms for logging and medication.
- User profile (age, typical cycle length, notes).
- Data export/import for healthcare visits or device changes.
- Light/dark theme support.

## Non-Goals / Constraints
- No native smartwatch or deep health platform integration in MVP.
- PWA push notifications are limited on iOS.
- Not a medical diagnosis tool; guidance is informational only.

## Success Metrics
- Active users logging cycles regularly.
- Accuracy of predictions improving over time.
- Low churn driven by privacy trust and usability.
- User satisfaction with symptom insights and advice.
